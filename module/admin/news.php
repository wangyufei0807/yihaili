...
switch ($act) {
	//#####################@ 文章增加 @#####################//
	case 'add':
		Core_Auth::checkauth("news_add"); //文章增加
		//商品分类
		include_once(PATH_ROOT.'hook/category.hook.php');
		$category_treelist = category_treelist('news');
		
		//文章标签
		$tag_arr = array();
		$$info['article_text']=htmlspecialchars($info['article_text']);
		$action = 'admin.php?mod=news&act=addsql';
		include($template->getfile('default/admin/news/news_add.html'));
	break;
	//#####################@ 文章增加sql @#####################//
	case 'addsql':
		Core_Auth::checkauth("news_add"); //文章增加
		...
		$_p_info['article_atime'] = $_p_info['article_atime'] ? strtotime($_p_info['article_atime']) : time();
			...
			$sql="insert into `".dbpre."news` (article_id,article_name,article_summary,article_text,article_logo,article_atime,article_clicknum,category_id,article_tuijian,article_cuxiao) values (NULL,'".$_p_info['article_name']."','".$_p_info['article_summary']."','".$_p_info['article_text']."','".$_p_info['article_logo']."','".$_p_info['article_atime']."','".$_p_info['article_clicknum']."','".$_p_info['category_id']."','".$_p_info['article_tuijian']."','".$_p_info['article_cuxiao']."')";

		if ($news_id = $db->insert($sql)) {
			//更新标签
			...
			echo "<script language='javascript'>alert('发布成功');window.location.href='admin.php?mod=news&act=list';</script>";
		}
		else {
	...
	//#####################@ 文章修改 @#####################//
	case 'edit':
		Core_Auth::checkauth("news_edit"); //编辑文章
		//文章分类
		include_once(PATH_ROOT.'hook/category.hook.php');
		$category_treelist = category_treelist('news');

		$sql  = "select * from `".dbpre."news` where article_id='$_g_id'";
		$info = $db->select($sql);

		$info['article_atime']=pe_date($info['article_atime']);
		$action = "admin.php?mod=news&act=editsql&id={$_g_id}";
		include($template->getfile('default/admin/news/news_add.html'));
	break;
	//#####################@ 文章修改sql @#####################//
	case 'editsql':
		...
		$sql="update `".dbpre."news` set article_name='".$_p_info['article_name']."',article_summary='".$_p_info['article_summary']."',article_text='".$_p_info['article_text']."',article_atime='".$_p_info['article_atime']."',category_id='".$_p_info['category_id']."',article_tuijian='".$_p_info['article_tuijian']."',article_cuxiao='".$_p_info['article_cuxiao']."'".$sqlwhere." where article_id='$_g_id'";

		if ($db->update($sql)) {
			echo "<script language='javascript'>alert('修改成功');window.location.href='admin.php?mod=news&act=list';</script>";
		}
		else {
	...
	case 'delsql':
		...
		if ($db->delete("delete from `".dbpre."news` where article_id in(".$new.")")) {
			echo "<script language='javascript'>alert('删除成功!');window.location.href='admin.php?mod=news&act=list';</script>";
		}
		else {
	...
	//#####################@ 文章列表 @#####################//
	default :
		Core_Auth::checkauth("news_list"); //查看文章
		//文章分类
		include_once(PATH_ROOT.'hook/category.hook.php');
		$category_treelist = category_treelist('news');
		...
		*/
		...
		$page=$db->page->html;
		include($template->getfile('default/admin/news/news_list.html'));
	break;
}
?>