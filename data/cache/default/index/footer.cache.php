<? if (!class_exists('template')) die('Access Denied');$template->getInstance()->check('default/index/footer.html', '98efa3310ab604c9d144bf00cbfe2cf0', 1435592453);?>
	<div class="content_l2">
        <div class="wb">
            <a href="/">
                <img src="<?=PATH_URL?>templates/default/index/images/back-home.png">
            </a>
            <img src="<?=PATH_URL?>templates/default/index/images/weixin.png">
        </div>
    </div>
	<div class="g-ft">
		<div class="g-in">
			<p>版权所有：青岛大桥海洋生物科技有限公司北京分公司 备案号：京ICP备15013724号</p>
		</div>
	</div>
</div>
<script type="text/javascript"> var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://"); document.write(unescape("%3Cscript src='" + _bdhmProtocol + "hm.baidu.com/h.js%3F7f5e16a363a0c8ba3e6231d51abe7af1' type='text/javascript'%3E%3C/script%3E")) </script>
</body>
</html>