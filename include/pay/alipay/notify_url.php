...
if($verify_result) {//验证成功
	...
	$sql  = "select * from `".dbpre."order` where  order_id='$out_trade_no'";
	$info = $db->select($sql);
	...
	//该判断表示买家已在支付宝交易管理中产生了交易记录且付款成功，但卖家没有发货
		if ($info['order_state'] == 'notpay') {
			$order_outid = $trade_no;
			$order_paytype = 'alipay_db';
			$order_state = 'paid';
			$order_ptime = time();	
			$sql="update `".dbpre."order` set order_outid='$order_outid',order_paytype='$order_paytype',order_state='$order_state',order_ptime='$order_ptime' where order_id='$out_trade_no'";
	        $sql=$db->update($sql);
	...
	//该判断表示卖家已经发了货，但买家还没有做确认收货的操作
		if ($info['order_state'] == 'paid') {
			$order_state = 'send';
			$order_stime = time();					
			$sql="update `".dbpre."order` set order_state='$order_state',order_stime='$order_stime' where order_id='$out_trade_no'";
	        $sql=$db->update($sql);
		}	
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
	...
	//该判断表示买家已经确认收货，这笔交易完成
		...
		elseif ($info['order_state'] == 'send') {
			$order_state = 'success';	
			$sql="update `".dbpre."order` set order_state='$order_state' where order_id='$out_trade_no'";
	        $sql=$db->update($sql);
		}		
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
